#!/usr/bin/env python
"""Create Borzoi mouse train/val/test splits."""
...
SPLIT_PATTERNS = {
    "split_chr10_chr11": {"val_chroms": ["chr10"], "test_chroms": ["chr11"]},
    "split_chr8_chr9": {"val_chroms": ["chr8"], "test_chroms": ["chr9"]},
    "split_chr14_chr15": {"val_chroms": ["chr14"], "test_chroms": ["chr15"]},
    "split_chr5_chr6": {"val_chroms": ["chr5"], "test_chroms": ["chr6"]},
}

TINY_SPLIT_PATTERNS = {
    "split_chr10_chr11_tiny50": {"base_split": "split_chr10_chr11", "step": 50},
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--chrom_sizes",
        default="/work/Database/Database_fromDocker/Referencedata_mm10/genometable.txt",
    )
...
def read_chrom_sizes(path: str) -> pd.DataFrame:
    chrom_sizes = pd.read_csv(path, sep="\t", header=None, names=["chrom", "size"])
    return chrom_sizes[chrom_sizes["chrom"].isin(AUTOSOMES)].copy()
...
def write_split(
    ...
    print(split_name, "train", len(train_intervals), "val", len(val_intervals), "test", len(test_intervals))
...
def main() -> None:
    ...
    for split_name, spec in SPLIT_PATTERNS.items():
        ...
        train_intervals = make_borzoi_intervals(train_chroms, chrom_sizes, SEQ_LEN, LABEL_LEN, STRIDE)
        val_intervals = make_borzoi_intervals(val_chroms, chrom_sizes, SEQ_LEN, LABEL_LEN, STRIDE)
        test_intervals = make_borzoi_intervals(test_chroms, chrom_sizes, SEQ_LEN, LABEL_LEN, STRIDE)
