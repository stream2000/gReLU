...
lora_log="${log_dir}/borzoi_${split_name}_${target_mode}_lora_3gpu_${timestamp}.log"
headonly_log="${log_dir}/borzoi_${split_name}_${target_mode}_headonly_3gpu_${timestamp}.log"

echo "Borzoi-style retrain started: $(date)"
echo "split_name=${split_name}"
echo "target_mode=${target_mode}"
echo "total_weight=${total_weight}"
echo "physical_gpus=${gpus}"
echo "lightning_devices=${devices}"
echo "lora_epochs=${lora_epochs}"
echo "headonly_epochs=${headonly_epochs}"
echo "early_stopping=true"
echo "patience=3"
...
echo "Borzoi-style retrain completed: $(date)"
